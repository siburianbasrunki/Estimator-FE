 ...
 *  ========================= */
async function setMasterOverrideActive(code: string, active: boolean) {
  const { hsp } = getEndpoints();
...
async function createMaster(payload: MasterCreatePayload): Promise<MasterItem> {
  const { hsp } = getEndpoints();
...
async function getMaster(id: string): Promise<MasterItem> {
  const { hsp } = getEndpoints();
...
async function updateMasterByCode(
  code: string,
  payload: MasterUpdatePayload
): Promise<MasterItem> {
  const { hsp } = getEndpoints();
  const res = await fetch(`${hsp}/master/by-code/${encodeURIComponent(code)}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json", ...authHeader() },
    body: JSON.stringify(payload),
  });
...
async function deleteMasterByCode(code: string): Promise<void> {
  const { hsp } = getEndpoints();
